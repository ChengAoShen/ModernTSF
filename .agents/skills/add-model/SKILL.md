...
3. Run `uv run tsf component match <requirements> --json` and inspect promising candidates with `component show`. Retrieval is only a shortlist: reuse a component only after its mathematics, shapes, normalization, masking, residual order, initialization, and outputs match. Keep unmatched paper-specific operations model-local. Replace the placeholder in `model.py`; never import another named model package as an implementation dependency.
