...
3. Run `uv run tsf component match <requirements> --json` and inspect promising candidates with `component show`. Read the returned component card and implementation. Retrieval is only a shortlist: reuse a component only after its mathematics, shapes, normalization, masking, residual order, initialization, and outputs match. Keep unmatched paper-specific operations model-local. Replace the placeholder in `model.py`; never import another named model package as an implementation dependency.
...
6. Run `uv run tsf model show MyModel`, `uv run tsf smoke --model MyModel`, and `uv run tsf repo doctor --strict --models MyModel`. Finish with `uv run tsf repo audit` so imports, component declarations, and cards remain synchronized.
