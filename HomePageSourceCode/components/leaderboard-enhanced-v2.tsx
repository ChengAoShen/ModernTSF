...
// Two-tier hierarchy with rich filtering
// ---------------------------------------------------------------------------
...
const CATEGORY_STRUCTURE = {
  ...
  realtime: {
    ...
    datasets: [] // dynamically populated
  }
};
...
export function LeaderboardEnhanced({
  ...
  // Update track when category changes
  React.useEffect(() => {
  ...
  // Dataset filters (8 core datasets)
  ...
  const stats = useMemo(() => {
    const counts: Record<string, number> = {
      ...
      Traffic: 0, Solar: 0, Electricity: 0, Weather: 0
    };

    allDatasets.forEach(([name]) => {
      const lower = name.toLowerCase();
      if (lower.includes("etth1")) counts.ETTh1++;
      else if (lower.includes("etth2")) counts.ETTh2++;
      else if (lower.includes("ettm1")) counts.ETTm1++;
      else if (lower.includes("ettm2")) counts.ETTm2++;
      else if (lower.includes("traffic")) counts.Traffic++;
      else if (lower.includes("solar")) counts.Solar++;
      else if (lower.includes("electricity")) counts.Electricity++;
      else if (lower.includes("weather")) counts.Weather++;
    });
  ...
  return (
    <div className="space-y-6">
            ...
            <button
              ...
              className={`rounded-xl border px-6 py-3 text-sm font-semibold transition-all ${
                active
                  ...
                  : "border-border bg-surface text-muted hover:text-ink hover:border-accent/50 hover:scale-102"
              }`}
            >
      ...
      <div className="rounded-2xl border border-border bg-surface p-5 space-y-5">
        ...
        {/* Dataset Categories - 8 core datasets */}
        {mainCategory === "common_static" && (
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <Checkbox
                checked={showETTh1}
                onChange={setShowETTh1}
                label={`ETTh1 (${stats.ETTh1})`}
                description="Hourly 1"
              />
              <Checkbox
                checked={showETTh2}
                onChange={setShowETTh2}
                label={`ETTh2 (${stats.ETTh2})`}
                description="Hourly 2"
              />
              <Checkbox
                checked={showETTm1}
                onChange={setShowETTm1}
                label={`ETTm1 (${stats.ETTm1})`}
                description="Minute 1"
              />
              <Checkbox
                checked={showETTm2}
                onChange={setShowETTm2}
                label={`ETTm2 (${stats.ETTm2})`}
                description="Minute 2"
              />
              <Checkbox
                checked={showTraffic}
                onChange={setShowTraffic}
                label={`Traffic (${stats.Traffic})`}
                description="Road traffic"
              />
              <Checkbox
                checked={showSolar}
                onChange={setShowSolar}
                label={`Solar (${stats.Solar})`}
                description="Solar power"
              />
              <Checkbox
                checked={showElectricity}
                onChange={setShowElectricity}
                label={`Electricity (${stats.Electricity})`}
                description="Power usage"
              />
              <Checkbox
                checked={showWeather}
                onChange={setShowWeather}
                label={`Weather (${stats.Weather})`}
        ...
        {/* Display Options */}
        <div>
          <p className="text-sm font-medium text-muted mb-3">Display</p>
        ...
        {/* Matrix display toggle */}
        <div>
          <p className="text-sm font-medium text-muted mb-3">Matrix</p>
          <Checkbox
            checked={matrixView}
            onChange={setMatrixView}
            label="Matrix view"
            description="Show all horizons in grid"
          />
        </div>

        {/* View mode */}
        <div>
          <p className="text-sm font-medium text-muted mb-3">Options</p>
      ...
      <div className="flex items-center justify-between text-sm text-muted">
        ...
        <span className="text-xs text-faint">
          {categoryInfo.label} · {TRACK_META[track as keyof typeof TRACK_META]?.label}
        </span>
      </div>
      ...
      {filteredDatasets.length === 0 ? (
        <p className="mt-10 rounded-2xl border border-border bg-surface px-6 py-16 text-center text-muted">
          {searchQuery ? `No datasets match "${searchQuery}"` : "No datasets in this track"}
        </p>
      ) : (
        <div className="space-y-6">
