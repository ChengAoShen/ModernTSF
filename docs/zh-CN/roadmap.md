...
全部 115 个模型，以及所有数据集、指标、损失、评估路径，都服务于这三种预测设定。`task.mode` 只暴露上述三种设定，因此**所有可达代码路径都是预测**。部分上游 TSLib 风格模型（Autoformer、FEDformer、TimesNet、TiDE、SegRNN、CrossLinear、MoFo）原本携带的多任务 `task_name` 分支已在移植时移除——代码库中不存在任何 `task_name` 模型参数、schema 字段或非预测分支。
