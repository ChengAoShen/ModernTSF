"""Verbatim DCRNN model source.

Vendored from CauAir (src/models/dcrnn.py).
...
def normalize_adj_mx(adj_mx, adj_type):
    """Normalize adjacency matrix and return as list of COO sparse matrices."""
    ...
    if adj_type == "doubletransition":
        ...
        return [_asym_adj(adj_mx)]
    elif adj_type == "identity":
        return [sp.coo_matrix(np.eye(adj_mx.shape[0], dtype=np.float32))]
    return []
...
class DiffusionGraphConv(nn.Module):
    ...
    def forward(self, inputs, supports, state, output_size, bias_start=0.0):
        ...
        x0 = torch.reshape(x0, shape=[self.node_num, input_size * batch_size])
...
class DCGRUCell(nn.Module):
    def __init__(self, device, node_num, input_dim, num_units, max_diffusion_step,
        ...
        self.dconv_gate = DiffusionGraphConv(
            node_num=node_num, supports_len=supports_len,
            input_dim=input_dim, hid_dim=num_units,
            output_dim=num_units * 2, max_diffusion_step=max_diffusion_step)
        self.dconv_candidate = DiffusionGraphConv(
            node_num=node_num, supports_len=supports_len,
            input_dim=input_dim, hid_dim=num_units,
            output_dim=num_units, max_diffusion_step=max_diffusion_step)
...
class DCRNNEncoder(nn.Module):
    def __init__(self, device, node_num, input_dim, hid_dim,
        ...
        encoding_cells = list()
        encoding_cells.append(DCGRUCell(
            device=device, node_num=node_num, input_dim=input_dim,
            num_units=hid_dim, max_diffusion_step=max_diffusion_step,
            filter_type=filter_type))
        for _ in range(1, num_rnn_layers):
            encoding_cells.append(DCGRUCell(
                device=device, node_num=node_num, input_dim=hid_dim,
                num_units=hid_dim, max_diffusion_step=max_diffusion_step,
                filter_type=filter_type))
...
class DCGRUDecoder(nn.Module):
    def __init__(self, device, node_num, input_dim, hid_dim, output_dim,
        ...
        self.num_rnn_layers = num_rnn_layers
        decoding_cells = list()
        decoding_cells.append(DCGRUCell(
            device=device, node_num=node_num, input_dim=input_dim,
            num_units=hid_dim, max_diffusion_step=max_diffusion_step,
            filter_type=filter_type))
...
class DCRNN(nn.Module):
    ...
    def __init__(self, device, adj_mx, node_num, input_dim, output_dim,
                 seq_len, horizon, n_filters=64, max_diffusion_step=2,
        ...
        self.encoder = DCRNNEncoder(
            device=device, node_num=node_num, input_dim=input_dim,
            hid_dim=n_filters, max_diffusion_step=max_diffusion_step,
            filter_type=filter_type, num_rnn_layers=num_rnn_layers)
        self.decoder = DCGRUDecoder(
            device=device, node_num=node_num, input_dim=output_dim,
            hid_dim=n_filters, output_dim=output_dim,
    ...
    def _calculate_supports(self, adj_mx, filter_type):
        supports = normalize_adj_mx(adj_mx, filter_type)
        results = []
        for support in supports:
    ...
    def forward(self, source, target=None, iter=None):
        """Forward pass. source: (B, T, N, F) -> (B, horizon, N, output_dim)"""
        ...
        if target is not None:
            ...
            target = go_symbol.expand(self.horizon + 1, -1, -1, -1).clone()

        init_hidden_state = self.encoder.init_hidden(b).to(self.device)
        context, _ = self.encoder(source, self.supports, init_hidden_state)

        if self.training and self.use_curriculum_learning and iter is not None:
