"""Independent LSTM forecasting baseline."""
...
class Model(nn.Module):
    """Encode a history with an LSTM and decode its final hidden state directly."""
    ...
    def __init__(self, seq_len: int, pred_len: int, enc_in: int, d_model: int = 64,
        ...
        self.encoder = nn.LSTM(enc_in, d_model, num_layers=num_layers, batch_first=True,
    ...
    def forward(self, x: torch.Tensor, *args: object) -> torch.Tensor:
        ...
        _, (final_hidden, _) = self.encoder(normalized)
        forecast = self.head(final_hidden[-1]).reshape(-1, self.pred_len, self.enc_in)
