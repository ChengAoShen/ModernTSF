"""Verbatim STGCN model source.

Vendored from CauAir (src/models/stgcn.py).
BaseModel replaced with nn.Module; explicit params stored on self.
Reference: https://github.com/hazdzz/STGCN
"""
...
class STGCN(nn.Module):
    ...
    def __init__(self, gso, blocks, Kt, Ks, dropout,
        ...
        for l in range(len(blocks) - 3):
            modules.append(STConvBlock(
                Kt, Ks, self.node_num,
                blocks[l][-1], blocks[l + 1], gso, dropout))
        self.st_blocks = nn.Sequential(*modules)
        Ko = self.seq_len - (len(blocks) - 3) * 2 * (Kt - 1)
        self.Ko = Ko
        if self.Ko > 1:
            self.output = OutputBlock(
                Ko, blocks[-3][-1], blocks[-2],
                blocks[-1][0], self.node_num, dropout)
        elif self.Ko == 0:
            self.fc1 = nn.Linear(
...
class OutputBlock(nn.Module):
    def __init__(self, Ko, last_block_channel, channels,
                 end_channel, node_num, dropout):
        ...
        self.fc2 = nn.Linear(
            in_features=channels[1], out_features=end_channel)
...
class TemporalConvLayer(nn.Module):
    def __init__(self, Kt, c_in, c_out, node_num):
        ...
        self.causal_conv = CausalConv2d(
            in_channels=c_in, out_channels=2 * c_out,
    ...
    def forward(self, x):
        if self.c_in > self.c_out:
            ...
            x = torch.cat([
                x,
                torch.zeros([
                    batch_size, self.c_out - self.c_in,
                    timestep, node_num]).to(x)
            ], dim=1)
        return x
...
class CausalConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size,
                 stride=1, enable_padding=False, dilation=1,
                 groups=1, bias=True):
        ...
        if enable_padding:
            self.__padding = [
                int((kernel_size[i] - 1) * dilation[i])
    ...
    def forward(self, input):
        if self.__padding != 0:
            input = F.pad(
                input,
                (self.left_padding[1], 0, self.left_padding[0], 0))
