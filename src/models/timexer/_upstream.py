"""Upstream TimeXer model from CauAir.
...
class TimeXer(nn.Module):
    def __init__(self, node_num: int, seq_len: int, horizon: int,
                 input_dim: int = 1, patch_len: int = 24,
                 d_model: int = 128, dropout: float = 0.1,
                 factor: int = 1, n_heads: int = 4,
                 e_layers: int = 2, d_ff: int = 256,
                 features: str = 'M'):
        ...
        self.n_vars = 1 if features == 'MS' else self.node_num

        # Embedding
        self.en_embedding = EnEmbedding(
            self.n_vars, d_model, self.patch_len, dropout)
        self.ex_embedding = DataEmbedding_inverted(
            seq_len * (2 * self.input_dim - 1), d_model)
        ...
        self.head = FlattenHead(
            self.n_vars, self.head_nf, self.pred_len,
    ...
    def forecast_multi(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        en_embed, n_vars = self.en_embedding(x_enc.permute(0, 2, 1))  # B L N -> B N L
        ex_embed = self.ex_embedding(x_enc, x_mark_enc)  # B L N
